...
export function parseAssessmentCsv(csv: string): {
  ...
  const semesterStr =
    semIdx !== -1 && semRow ? semRow[semIdx + 2]?.trim() ?? "" : "";
  ...
  const courseRow = rows.find((r) =>
    r.some((c) => c?.includes("Course & Sec"))
  );
  const courseIdx = courseRow
    ? courseRow.findIndex((c) => c?.includes("Course & Sec"))
    : -1;
  const rawCourseSection =
    courseIdx !== -1 && courseRow ? courseRow[courseIdx + 2]?.trim() ?? "" : "";
  ...
  const studentHeaderIndex = rows.findIndex((r) =>
    r.includes("Name of Students")
  );
  ...
  const achievedRow = rows.find((r) =>
    r.some((c) => c?.includes("ACHIEVED THE MINIMUM"))
  );
